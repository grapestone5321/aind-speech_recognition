...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    input_deep = input_data
    for i in range(recur_layers):
           deep_rnn = GRU(units, activation='relu', return_sequences=True, implementation=2)(input_deep)
           bn_rnn = BatchNormalization()(deep_rnn)
           input_deep = bn_rnn
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(input_deep)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
...
def final_model(input_dim, filters, kernel_size, conv_stride,
    ...
    bn_rnn = bn_cnn
    for i in range(recur_layers):
        bidir_rnn = Bidirectional(GRU(units, activation='relu', return_sequences=True, implementation=2, dropout=0.2))(bn_rnn)
